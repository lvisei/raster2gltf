...
def stl_to_gltf(path_to_stl, out_path, is_binary):
    ...
    with open(path_to_stl, "rb") as f:
        ...
        stl_assume_bytes = header_bytes + unsigned_long_int_bytes + \
            number_faces * (vec3_bytes*3 + spacer_bytes + vec3_bytes)
        assert stl_assume_bytes == os.path.getsize(
            path_to_stl), "stl is not binary or ill formatted"
    ...
    # each vec3 has 3 floats, each float is 4 bytes
    ...
    glb_out.extend(struct.pack('%df' % number_vertices*3, *
                               flatten(vertices)))  # magin number for BIN
